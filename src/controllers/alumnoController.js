...
const getOneAlumno = async(req, res) => {
  const alumnos = await alumnoProceso.getOneAlumnos(req.params.alumnoId); 
  res.send(alumnos); 
};
...
const updateOneAlumno = async (req, res) => {
  const alumnos = await alumnoProceso.updateOneAlumno(req.body.nombre, req.body.apellido, req.body.telefono, req.body.direccion);
  res.send(alumnos); 
};

const deleteOneAlumno = async (req, res) => {
  const alumnos = await alumnoProceso.deleteOneAlumnos (req.params.alumnoId); 
  res.send(alumnos); 
};
